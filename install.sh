#!/usr/bin/env bash
# ai-limit menubar app — 一键安装脚本（macOS）
#
# 用法：
...
#
# 注意：菜单栏 App 需仓库提供 DMG Release。若尚无 Release，请改用 CLI（见 README）。

set -euo pipefail

REPO="aguithub/ai-limit"
...
# ── 环境检查 ─────────────────────────────────────────────────────────────────
[[ "$(uname)" == "Darwin" ]] || die "仅支持 macOS（其它平台请使用 CLI：python3 usage.py）"
...
[[ -n "$VERSION" ]] || die "无法获取版本号（仓库可能尚未发布 Release），请改用 CLI"
